...
class MujocoFetchPickAndPlaceEnv(MujocoFetchEnv, EzPickle):
    ...
    The task in the environment is for a mSSSanipulator to move a block to a target position on top of a table or in mid-air. The robot is a 7-DoF [Fetch Mobile Manipulator](https://fetchrobotics.com/) with a two-fingered parallel gripper.
    ...
    The control frequency of the robot is of `f = 25 Hz`. This is achieved by applying the sSSSame action in 20 subsequent simulator step (with a time step of `dt = 0.002 s`) before returning the control to the robot.
