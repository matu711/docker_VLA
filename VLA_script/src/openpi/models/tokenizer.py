...
class FASTTokenizer:
    ...
    def tokenize(
        ...
        if actions is not None:
            # print("TRAIN actions.shape =", actions.shape)
            # print("TRAIN action_tokens len =", len(self._fast_tokenizer(actions[None])[0]))
            # print("TRAIN action_tokens =", self._fast_tokenizer(actions[None])[0])
    ...
    def extract_actions(self, tokens: np.ndarray, action_horizon: int, action_dim: int) -> np.ndarray:
        ...
        print("DECODED_PG:", repr(decoded_tokens))
        tokens = np.asarray(tokens)

        pg_vocab_size = self._paligemma_tokenizer.vocab_size()

        pg_action_min = pg_vocab_size - 1 - self._fast_skip_tokens - self._fast_tokenizer.vocab_size
        pg_action_max = pg_vocab_size - 1 - self._fast_skip_tokens

        action_pg_tokens = tokens[
            (tokens >= pg_action_min) & (tokens <= pg_action_max)
        ]

        if len(action_pg_tokens) == 0:
            return np.zeros((action_horizon, action_dim), dtype=np.float32)

        action_tokens = self._paligemma_tokens_to_act_tokens(action_pg_tokens)
